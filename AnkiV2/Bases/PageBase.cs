...
namespace AnkiV2.Bases
{
    public class PageBase
    {
        ...
        public PageBase()
        {
            wait = new OpenQA.Selenium.Support.UI.WebDriverWait(DriverFactory.INSTANCE, TimeSpan.FromSeconds(GlobalParameters.CONFIG_DEFAULT_TIMEOUT_IN_SECONDS));
        ...
        protected void Click(By locator)
        {
            ...
            while (timeOut.Elapsed.Seconds <= GlobalParameters.CONFIG_DEFAULT_TIMEOUT_IN_SECONDS)
            {
                try
                {
        ...
        {
            WaitForElement(locator).SendKeys(Keys.Control + "a");
            WaitForElement(locator).SendKeys(Keys.Delete);
        ...
        protected void SendKeysNoReport(By locator, string text)
        {
            Clear(locator);
            WaitForElement(locator).SendKeys(text);
        ...
        protected void WaitForElementNoExists(By locator)
        {
            wait.Until(ExpectedConditions.InvisibilityOfElementLocated(locator));
        }

        protected void ClickUsingTab(By locator)
        {
        ...
        {
            WaitForElement(locator).SendKeys(Keys.Space);
            ExtentReportHelpers.AddTestInfo(3, "");
        }
